...
// Define alpha parameter for work distribution (0.0 to 1.0)
#define ALPHA 0.7 // 70% on GPU, 30% on CPU
...
void cl_launch_kernel(int nx, int ny)
{
	...
	// Adjust global work size for the reduced number of rows
...
void bicg_cpu(int nx, int ny, int start_row, DATA_TYPE POLYBENCH_2D(A,NX,NY,nx,ny), DATA_TYPE POLYBENCH_1D(r,NX,nx), 
              DATA_TYPE POLYBENCH_1D(s,NY,ny), DATA_TYPE POLYBENCH_1D(p,NY,ny), DATA_TYPE POLYBENCH_1D(q,NX,nx))
{
	int i,j;
	
  	for (i = start_row; i < _PB_NY; i++)
	{
		s[i] = 0.0;
		for (j = 0; j < nx; j++)
		{
...
int main(int argc, char *argv[]) 
{
	...
	// Calculate split point based on alpha
	...
	// Execute GPU portion (first gpu_rows rows)
	printf("Executing on GPU from row %d to %d\n", gpu_start, gpu_end);
	cl_launch_kernel(gpu_rows, ny);

	errcode = clEnqueueReadBuffer(clCommandQue, s_mem_obj, CL_TRUE, 0, gpu_rows*sizeof(DATA_TYPE), POLYBENCH_ARRAY(s), 0, NULL, NULL);
	errcode = clEnqueueReadBuffer(clCommandQue, q_mem_obj, CL_TRUE, 0, gpu_rows*sizeof(DATA_TYPE), POLYBENCH_ARRAY(q), 0, NULL, NULL);
	if(errcode != CL_SUCCESS) printf("Error in reading GPU mem\n");

	// Execute CPU portion (remaining rows)
	printf("Executing on CPU from row %d to %d\n", cpu_start, nx);
	bicg_cpu(nx, ny, gpu_rows, POLYBENCH_ARRAY(A), POLYBENCH_ARRAY(r), POLYBENCH_ARRAY(s), 
		...
		DATA_TYPE *s_cpu = (DATA_TYPE*)malloc(ny*sizeof(DATA_TYPE));
		DATA_TYPE *q_cpu = (DATA_TYPE*)malloc(nx*sizeof(DATA_TYPE));
		memcpy(s_cpu, POLYBENCH_ARRAY(s), ny*sizeof(DATA_TYPE));
		memcpy(q_cpu, POLYBENCH_ARRAY(q), nx*sizeof(DATA_TYPE));
		
		bicg_cpu(nx, ny, 0, POLYBENCH_ARRAY(A), POLYBENCH_ARRAY(r), s_cpu, POLYBENCH_ARRAY(p), q_cpu);
		...
		compareResults(nx, ny, s_cpu, POLYBENCH_ARRAY(s), q_cpu, POLYBENCH_ARRAY(q));
