...
void cl_launch_kernel(int gpu_rows, int n, DATA_TYPE alpha, DATA_TYPE beta)
{
...
int main(int argc, char *argv[])
{
	...
	if (cpu_rows > 0) {
		...
		gesummv_partial(cpu_start, cpu_end, n, alpha, beta, POLYBENCH_ARRAY(A), POLYBENCH_ARRAY(B), POLYBENCH_ARRAY(tmp), POLYBENCH_ARRAY(x), POLYBENCH_ARRAY(y_outputFromGpu));

	}

	if (gpu_rows > 0) {
		printf("Running GPU computation for rows %d to %d...\n", gpu_start, gpu_end);
		cl_launch_kernel(gpu_rows, n, alpha, beta);

		errcode = clEnqueueReadBuffer(clCommandQue, y_mem_obj, CL_TRUE, 0, gpu_rows*sizeof(DATA_TYPE), POLYBENCH_ARRAY(y_outputFromGpu), 0, NULL, NULL);
	...
	size_t b1_size = 2 * sizeof(DATA_TYPE) * N * N;
	size_t b2_size = 3 * sizeof(DATA_TYPE) * N;

	size_t buffer_size = b1_size + b2_size;
	size_t arg_size = 2*sizeof(DATA_TYPE) + sizeof(int);
