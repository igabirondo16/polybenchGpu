...
void cl_launch_kernel(int gpu_rows, int ni, int nj, int nk, DATA_TYPE alpha, DATA_TYPE beta)
{
...
void gemm_partial(int start_row, int end_row, int ni, int nj, int nk, DATA_TYPE alpha, DATA_TYPE beta, DATA_TYPE POLYBENCH_2D(A,NI,NK,ni,nk),
	 DATA_TYPE POLYBENCH_2D(B,NK,NJ,nk,nj), DATA_TYPE POLYBENCH_2D(C,NI,NJ,ni,nj))
{
	int i,j,k;
...
int main(int argc, char *argv[])
{
	...
	if (cpu_rows > 0) {
		...
		gemm_partial(cpu_start, cpu_end, ni, nj, nk, alpha, beta, POLYBENCH_ARRAY(A), POLYBENCH_ARRAY(B), POLYBENCH_ARRAY(C_outputFromGpu));

	}

	if (gpu_rows > 0) {
		printf("Running GPU computation for rows %d to %d...\n", gpu_start, gpu_end);
		cl_launch_kernel(gpu_rows, ni, nj, nk, alpha, beta);
	...
	size_t c_size = sizeof(DATA_TYPE) * NI * NJ;

	size_t buffer_size = a_size + b_size + c_size;
	size_t arg_size = 2*sizeof(float) + 3*sizeof(int);
