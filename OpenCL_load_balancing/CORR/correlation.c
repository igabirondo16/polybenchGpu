...
void cl_launch_kernel1(
	...
	if (gpu_rows > 0) {
		...
		localWorkSize_Kernel1[0] = DIM_LOCAL_WORK_GROUP_KERNEL_1_X;
		localWorkSize_Kernel1[1] = DIM_LOCAL_WORK_GROUP_KERNEL_1_Y;
		globalWorkSize_Kernel1[0] = (size_t)ceil(((float)gpu_rows) / ((float)DIM_LOCAL_WORK_GROUP_KERNEL_1_X)) * DIM_LOCAL_WORK_GROUP_KERNEL_1_X;
		globalWorkSize_Kernel1[1] = 1;
	...
	if (gpu_rows > 0) {
		...
		errcode = clEnqueueReadBuffer(clCommandQue, mean_mem_obj, CL_TRUE, 0, gpu_rows * sizeof(DATA_TYPE), mean, 0, NULL, NULL);
...
void cl_launch_kernel2(
	...
	if (gpu_rows > 0) {
		...
		localWorkSize_Kernel2[0] = DIM_LOCAL_WORK_GROUP_KERNEL_2_X;
		localWorkSize_Kernel2[1] = DIM_LOCAL_WORK_GROUP_KERNEL_2_Y;
		globalWorkSize_Kernel2[0] = (size_t)ceil(((float)gpu_rows) / ((float)DIM_LOCAL_WORK_GROUP_KERNEL_2_X)) * DIM_LOCAL_WORK_GROUP_KERNEL_2_X;
		globalWorkSize_Kernel2[1] = 1;
	...
	if (gpu_rows > 0) {
		...
		errcode = clEnqueueReadBuffer(clCommandQue, stddev_mem_obj, CL_TRUE, 0, gpu_rows * sizeof(DATA_TYPE), stddev, 0, NULL, NULL);
		if(errcode != CL_SUCCESS) printf("Error in reading GPU mem\n");
...
void cl_launch_kernel3(
	...
	if (gpu_rows > 0) {
		...
		localWorkSize_Kernel3[1] = DIM_LOCAL_WORK_GROUP_KERNEL_3_Y;
		globalWorkSize_Kernel3[0] = (size_t)ceil(((float)M) / ((float)DIM_LOCAL_WORK_GROUP_KERNEL_3_X)) * DIM_LOCAL_WORK_GROUP_KERNEL_3_X;
		globalWorkSize_Kernel3[1] = (size_t)ceil(((float)gpu_rows) / ((float)DIM_LOCAL_WORK_GROUP_KERNEL_3_Y)) * DIM_LOCAL_WORK_GROUP_KERNEL_3_Y;
	...
	if (gpu_rows > 0) {
		...
		errcode = clEnqueueReadBuffer(clCommandQue, data_mem_obj, CL_TRUE, 0, gpu_rows * N * sizeof(DATA_TYPE), data, 0, NULL, NULL);
		if(errcode != CL_SUCCESS) printf("Error in reading GPU mem\n");
...
void cl_launch_kernel4(
	...
	if (gpu_rows > 0) {
		...
		localWorkSize_Kernel4[0] = DIM_LOCAL_WORK_GROUP_KERNEL_4_X;
		localWorkSize_Kernel4[1] = DIM_LOCAL_WORK_GROUP_KERNEL_4_Y;
		//if (end_cpu < m) gpu_rows += 1;
		globalWorkSize_Kernel4[0] = (size_t)ceil(((float)gpu_rows) / ((float)DIM_LOCAL_WORK_GROUP_KERNEL_4_X)) * DIM_LOCAL_WORK_GROUP_KERNEL_4_X;
		globalWorkSize_Kernel4[1] = 1;
	...
	// Calculate the m * m correlation matrix. 
  	for (j1 = start_cpu; j1 < end_cpu-1; j1++)
	{	
		symmat[j1][j1] = 1.0;
	...
	if (gpu_rows > 0) {
        ...
        if (gpu_rows < m - 1) gpu_rows += 1;
		errcode = clEnqueueReadBuffer(clCommandQue, symmat_mem_obj, CL_TRUE, 0, gpu_rows * M * sizeof(DATA_TYPE), symmat, 0, NULL, NULL);
		if(errcode != CL_SUCCESS) printf("Error in reading GPU mem\n");

		// Mirror upper triangle into lower on CPU
		for (int j1 = 0; j1 < gpu_rows; j1++) {
...
void correlation_collab(
	...
	cl_launch_kernel1(start_cpu, end_cpu, gpu_rows, m, n, data, mean);
	cl_launch_kernel2(start_cpu, end_cpu, gpu_rows, m, n, data, mean, stddev);
	cl_launch_kernel3(start_cpu, end_cpu, gpu_rows, m, n, data, mean, stddev);
	cl_launch_kernel4(start_cpu, end_cpu, gpu_rows, m, n, data, symmat);
}
