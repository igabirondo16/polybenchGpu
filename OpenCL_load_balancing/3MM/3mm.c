...
void cl_launch_kernel1(
	...
	DATA_TYPE POLYBENCH_2D(A,NJ,NL,nj,nl),
	...
	if (gpu_rows > 0) {
		...
		errcode = clEnqueueReadBuffer(clCommandQue, e_mem_obj, CL_TRUE, 0, sizeof(DATA_TYPE) * gpu_rows * NL, E, 0, NULL, NULL);
...
void cl_launch_kernel2(
	...
	if (gpu_rows > 0) {
		...
		errcode = clEnqueueReadBuffer(clCommandQue, f_mem_obj, CL_TRUE, 0, sizeof(DATA_TYPE) * gpu_rows * NL, F, 0, NULL, NULL);
		if(errcode != CL_SUCCESS) printf("Error in reading GPU mem\n");
...
void cl_launch_kernel3(
	...
	int nl,
	int nj,
	DATA_TYPE POLYBENCH_2D(G_output,NI,NL,ni,nl),
	DATA_TYPE POLYBENCH_2D(E,NI,NJ,ni,nj),
	...
	for (i = cpu_start; i < cpu_end; i++)
	{
		for (j = 0; j < _PB_NL; j++)
		{
			...
			for (k = 0; k < _PB_NJ; ++k)
			{
				G_output[i][j] += E[i][k] * F[k][j];
			}
		}
	}

	if (gpu_rows > 0) {
		errcode = clEnqueueReadBuffer(clCommandQue, g_mem_obj, CL_TRUE, 0, sizeof(DATA_TYPE) * gpu_rows * NL, G_output, 0, NULL, NULL);
		if(errcode != CL_SUCCESS) printf("Error in reading GPU mem\n");
...
void cl_launch_kernel(int gpu_rows, int ni, int nj, int nk, int nl, int nm)
{
	size_t localWorkSize[2], globalWorkSize[2];
	localWorkSize[0] = DIM_LOCAL_WORK_GROUP_X;
	localWorkSize[1] = DIM_LOCAL_WORK_GROUP_Y;
...
void mm3_collab(int gpu_rows, int start_cpu, int end_cpu, int ni, int nj, int nk, int nl, int nm,
		DATA_TYPE POLYBENCH_2D(E,NI,NJ,ni,nj),
		DATA_TYPE POLYBENCH_2D(A,NI,NK,ni,nk),
		DATA_TYPE POLYBENCH_2D(B,NK,NJ,nk,nj),
		DATA_TYPE POLYBENCH_2D(F,NJ,NL,nj,nl),
		...
		DATA_TYPE POLYBENCH_2D(G,NI,NL,ni,nl))
{
	...
	/* E := A*B */
	cl_launch_kernel1(start_cpu, end_cpu, gpu_rows, ni, nj, nk, E, A, B);

	/* F := C*D */
	cl_launch_kernel2(start_cpu, end_cpu, gpu_rows, nj, nl, nm, F, C, D);

	/* G := E*F */
	cl_launch_kernel3(start_cpu, end_cpu, gpu_rows, ni, nl, nj, G, E, F);
}
